...
#' A function for applying a basCreditsafe theme to a ggplot
#'
#' This function applies bas theme to a ggplot.
...
theme_ycs <- function(type = c("grid", "horizontal", "vertical", "blank")) {
  return(theme_bas(type))
}

#' A function for applying a bas theme to a ggplot
#'
#' This function applies bas theme to a ggplot.
#' The first time it is used it will create a column specification file
#' allows you to create and or read a file which you can use to control a data file import
...
#' @examples
#' theme_bas("grid")
theme_bas <- function(type = c("grid", "horizontal", "vertical", "blank")) {
  ...
  bas_theme <-
    ...
    ggplot2::theme(axis.title = element_text(face = "plain"),
          panel.grid.major = element_line(colour = col_bas_grid),
          plot.background = element_blank(),
          axis.line.x = element_line(colour = col_bas_axis),
          axis.line.y = element_line(colour = col_bas_axis),
...
#' A function to include in a ggplot so the col aesthetics make use of the bas color palette
#'
#' @keywords ggplot2
#' @export
scale_color_bas <- function(...){
  return(ggplot2::scale_color_manual(values = col_bas, ...))
}


#' A function to include in a ggplot so the col aesthetics make use of the basCreditsafe color palette
#'
#' @keywords ggplot2
#' @export
...
#' A function to include in a ggplot so the fill aesthetics make use of the bas color palette
#'
#' @keywords ggplot2
#' @export
scale_fill_bas <- function(...){
  return(ggplot2::scale_fill_manual(values = col_bas, ...))
}

#' A function to include in a ggplot so the fill aesthetics make use of the basCreditsafe color palette
#'
#' @keywords ggplot2
#' @export
...
#' A function to include in a ggplot so color and fill aesthetics make use of the bas color palette
#'
#' @keywords ggplot2
#' @export
scale_gradient_bas <- function(...){
  return(ggplot2::scale_fill_gradient(low = col_bas_low, high = col_bas_high) +
           ggplot2::scale_color_gradient(low = col_bas_low, high = col_bas_high), ...)
}

#' A function to include in a ggplot so color and fill aesthetics make use of the basCreditsafe color palette
#'
#' @keywords ggplot2
#' @export
